...
namespace DeeGateway.Configuration.Controller
{
    [Options(AllowOrigin = "*", AllowHeaders = "*")]
        [Controller(BaseUrl = "__manage/plugin/rewrite")]
        [TokenFilter]
    public class Rewrite
    {
        ...
        public JsonResult ListServer(IHttpContext context, int page, int limit, string sortField, string orderType, int urlroute_id, int enable)
        {
            ...
            var JwtAuthList = pluginService.GetRule<rewrite>(enable, page, limit, totalCount, urlroute_id).Result;

            var JwtAuthExtList = JwtAuthList.BuildAdapter().AdaptToType<List<rewrite_rule_ext>>();
        ...
        [Post]
        public JsonResult RewriteInfo(IHttpContext context, int id)
        {
            var ret = new LayuiTableResultDTO();

            PluginService pluginService = new PluginService();
            var PluginList = pluginService.GetRule<rewrite>(id).Result;

            var PluginExtList = PluginList.BuildAdapter().AdaptToType<rewrite_rule_ext>();
        ...
        [Post]
        public JsonResult Update(IHttpContext context, rewrite body)
        {
            var ret = new LayuiTableResultDTO();
        ...
        [Post]
        public JsonResult Insert(IHttpContext context, rewrite body)
        {
            var ret = new LayuiTableResultDTO();
        ...
        [Post]
        public JsonResult Delete(IHttpContext context, int[] body)
        {
            var ret = new LayuiTableResultDTO();
            PluginService pluginService = new PluginService();
            var retCount = pluginService.DeleteRule<rewrite>(body);
            ret.code = retCount > 0 ? 0 : 1;
            ret.data = null;
            ret.count = retCount;
            return new JsonResult(ret);
        ...
        public JsonResult SyncCache(IHttpContext context, string name)
        {
            ...
            PluginService pluginService = new PluginService();
            var List = pluginService.GetRule<rewrite>(Enums.IsEnable.ENABLE, 1, 1000).Result;
            var ExtList = List.BuildAdapter().AdaptToType<List<rewrite_rule_ext>>();
